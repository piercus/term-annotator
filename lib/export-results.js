...
module.exports = function (options) {
	...
	return Promise.all(classNames.map(cN => {
			...
			return Promise.all(dirs.map(dir => {
				...
				if (options.classes[cN].params) {
					options.classes[cN].params.forEach((param, index) => {
						if ((index !== 0 && options.classes[cN].params.length > 1) || options.classes[cN].params.length === 1) {
							const choices = [];
							data.forEach(f => {
								let thumbnail;
								let files;
					...
					let thumbnail;
					const thumbnailsDir = data[0].filenames.filter(file => file.endsWith('thumbnails'));
					if (thumbnailsDir.length === 0) {
						thumbnail = null;
					} else if (thumbnailsDir.length === 1) {
						const thumbnails = fs.readdirSync(thumbnailsDir[0]).map(item => `${thumbnailsDir}/${item}`);
		...
		const savePath = dialog.showSaveDialogSync({
			defaultPath: '~/annotation.json', // Implement like in wiki
