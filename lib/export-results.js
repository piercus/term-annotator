		...
		return getDirnames(options.classes[cN]).then(dirs => {
			return Promise.all(dirs.map(dir => {
				return readDir(dir).catch(error => { // eslint-disable-line no-unused-vars
					...
					return {filenames, dir: escapeDirName(dir)};
				});
			})).then(data => {
				const sub = [];
							...
							data.forEach(f => {
								...
								if (thumbnailsDir.length === 0) {
									...
									if (thumbnails.length === 0) {
										...
										thumbnail = escapeDirName(thumbnails[0]);
									}
...
const escapeDirName = function (filenames) {
	if (Array.isArray(filenames)) {
		return filenames.map(f => f.match(/[-_.0-9a-z]*$/)[0]);
	}

	return filenames.match(/[-_.0-9a-z]*$/)[0];
};
