...
module.exports = function (options, queue) {
	...
	return Promise.all([
		...
		Promise.all(
			classNames.map(cN => {
				return getDirnames(options.classes[cN], options.destination.dir).then(dirs => {
					return Promise.all(dirs.map(() => {
						return readDir(options.destination.dir + '/' + options.classes[cN].dir).catch(error => { // eslint-disable-line no-unused-vars
							// Console.log(error);
							return [];
						});
					})).then(filenames => {
						...
						perClass[cN] = {
							...
							description: options.classes[cN].description,
