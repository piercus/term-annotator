...
module.exports = function (config) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!fs.existsSync(thumbnailsDir)) {
				...
				stream.inputOptions(['-ss 00:00:00'])
					...
					.save(`${thumbnailsDir}/${thumbnailName}`)
	...
	*/
	...
	ipcMain.on('export-json', event => {
		console.log('server, export results');
		return exportResults(config).then(res => {
			event.sender.send('export', res);
		}, err => {
			console.log(err);
			event.sender.send('error', err);
		});
	...
	ipcMain.on('import-json', event => {
		...
		return importResults().then(res => {
		//	Const updatedConfig = store.get('config')
			event.sender.send('import', res);
		}, err => {
			console.log(err);
			event.sender.send('error', err);
		});
