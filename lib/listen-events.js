...
module.exports = function (config) {
	...
	*/
	const onCategorize = function (opts) {
		...
		const thumbnails = files.filter(f => f.endsWith('.png'))
		return new Promise((resolve, reject) => {
			if (thumbnails.length === 0) {
				// gm(opts.media)
				// 	.selectFrame(0)
				// 	.write(dirname + '/' + name + '.png', function (err) {
				// 		if (err) reject(err);
				// 		resolve()
				// 	})
				gifFrames({ url: opts.media, frames: 0 }, function (err, frameData) {
					if (err) reject(err);
					frameData[0].getImage().pipe(fs.createWriteStream(dirname + '/' + name + '.png'));
					resolve()
				})
