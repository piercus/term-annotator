								...
								if (index === 0) {
									...
									filenames.forEach((f, ind) => {
										...
										if (thumbnailsPerChoice.length === 1) {
											...
											thumbnails = null; numFiles = f.length
										}
										const key = param.choices[ind]?param.choices[ind].key:param.choices[ind-param.choices.length].key
										const value = param.choices[ind]?param.choices[ind].key:param.choices[ind-param.choices.length].key
										const label = param.choices[ind]?param.choices[ind].key:param.choices[ind-param.choices.length].key
										choices.push({
											key,
											value,
											label,
