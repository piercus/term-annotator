...
const nextThumbnail = function (classDir) { // eslint-disable-line no-unused-vars
	changeStatus('import');
	const opts = {classDir};
	ipcRenderer.send('next-thumbnail', opts);
};
...
ipcRenderer.on('all-info', (event, arg) => {
	...
	classNames.forEach((cN, index) => {
		...
		if (arg.classes[cN].thumbnail) {
			let thumbnail;
			if (arg.classes[cN].thumbnail.startsWith('/')) {
				thumbnail = arg.classes[cN].thumbnail;
			} else {
				thumbnail = path.join(process.cwd(), arg.classes[cN].thumbnail);
			}

			const classDir = arg.classes[cN].thumbnail.replace(/thumbnails\/[-_.0-9a-z]*$/, '');
			fourthRow += `<td > \
            <img id=im-${index} src='${thumbnail}' style="width:250px;height:300px !important;"> \
            <a href="#"  id=${classDir} onclick="nextThumbnail(id)" class="next">Next Image &raquo;</a> \
            </td>`;
		} else {
		...
		if (arg.params[pN].sub[0]) {
			if (arg.params[pN].main) {
				...
				arg.params[pN].main.choices.forEach(ch => {
					fifthRow += `<td colspan="${sharedChoisesNum}">&nbsp;${arg.params[pN].main.name}-${ch.label}</td>`;
				});
			}
			...
			arg.params[pN].sub[0].choices.forEach((choice, index) => {
				...
				if (choice.thumbnail) {
					...
					if (choice.thumbnail.startsWith('/')) {
						...
						thumbnail = path.join(process.cwd(), choice.thumbnail);
					}
	...
	const message = `<table id='categories'><tr>${firstRow}</tr><tr>${secondRow}</tr><tr>${thirdRow}</tr><tr>${fourthRow}</tr></table>`;
	const message2 = `<table id='categories'><tr>${fifthRow}</tr><tr>${sixthRow}</tr><tr>${seventhRow}</tr><tr>${eighthRow}</tr></table>`;
